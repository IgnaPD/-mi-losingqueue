...
const REGIONS = { euw: ['euw1', 'europe'], eune: ['eun1', 'europe'], na: ['na1', 'americas'], kr: ['kr', 'asia'], las: ['la2', 'americas'], lan: ['la1', 'americas'] };
...
export function makeClient(key, region = 'euw') {
  ...
  async function api(host, path) {
    ...
    if (res.status === 403 || res.status === 401) throw new Error('Clave de Riot API inválida o vencida — las claves de desarrollo duran 24h. Conseguí una nueva en developer.riotgames.com y pegala en el campo de clave.');
...
export const tierStr = e => e ? `${e.tier[0]}${e.tier.slice(1).toLowerCase()} ${e.rank} ${e.leaguePoints}LP` : 'Sin rango';
...
export async function analyzeMatch(c, db, { name, tag, puuid, matchId }) {
  ...
  return {
    ...
    players: rows.map(r => ({ n: r.name, team: r.team === 100 ? 'blue' : 'red', rank: r.rank, pos: r.pos, champ: r.champ, kda: r.kda, dmg: r.dmg, cs: r.cs, cspm: +(r.cs / mins).toFixed(1), ga: r.ga.ga, form: `${r.ga.wins}W-${r.ga.n - r.ga.wins}L`, streak: r.ga.streak, wr: r.ga.wr, seasonGames: r.ga.seasonGames, duo: duoNames.has(r.name), duoWith: duoWith[r.name] || null, duoShared: duoShared[r.name] ?? null, duoRecord: duoRec[r.name] || null, deniedChamp: r.ga.deniedChamp || null, masteryPts: r.ga.masteryPts || 0, flags: [!r.ga.smurf && r.ga.autofill && 'autofill', r.ga.otp && 'otp', !r.ga.otp && r.ga.masteryPts >= 150000 && 'mastery', r.ga.otpDenied && 'otp-denied', !r.ga.smurf && r.ga.tilt && 'tilt', r.ga.rusty && 'rusty', r.ga.smurf && 'smurf', r.ga.afkRisk && 'afk-risk'].filter(Boolean), place: r.place || null, badge: r.badge || null, perfScore: r.perfScore ?? null, perfDetail: r.perfDetail || null })),
  };
}
...
export async function analyzeLive(c, db, { name, tag, puuid }) {
  ...
  for (const p of parts) {
    ...
    if (!p.puuid) {
      rows.push({ name: p.riotId || 'Unknown', team: p.teamId, pos: '', champ: champs[String(p.championId)] || 'Unknown', rank: 'Sin rango', tierN: null, ga: null });
      continue;
    }
  ...
  const entry = {
    ...
    players: rows.map(r => ({ n: r.name, team: r.team === 100 ? 'blue' : 'red', rank: r.rank, pos: r.pos, champ: r.champ, ga: r.ga ? r.ga.ga : null, form: r.ga ? `${r.ga.wins}W-${r.ga.n - r.ga.wins}L` : null, streak: r.ga ? r.ga.streak : null, wr: r.ga ? r.ga.wr : null, seasonGames: r.ga ? r.ga.seasonGames : null, duo: duoNames.has(r.name), duoWith: duoWith[r.name] || null, duoShared: duoShared[r.name] ?? null, duoRecord: duoRec[r.name] || null, deniedChamp: r.ga ? (r.ga.deniedChamp || null) : null, masteryPts: r.ga ? (r.ga.masteryPts || 0) : 0, flags: r.ga ? [!r.ga.smurf && r.ga.autofill && 'autofill', r.ga.otp && 'otp', !r.ga.otp && r.ga.masteryPts >= 150000 && 'mastery', r.ga.otpDenied && 'otp-denied', !r.ga.smurf && r.ga.tilt && 'tilt', r.ga.rusty && 'rusty', r.ga.smurf && 'smurf', r.ga.afkRisk && 'afk-risk'].filter(Boolean) : [], place: null, badge: null, perfScore: null, perfDetail: null, posInferred: true })),
  };
